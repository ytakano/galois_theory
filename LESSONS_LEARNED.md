...
CLAUDE.mdに、証明する際に必要ないファイルやディレクトリを明示的に記載することにしました。これにより、証明に集中できるようになると考えました。具体的には、以下のファイルやディレクトリを明示的に記載することにしました。推論に不要なファイルは、[.claudeignore](.claudeignore)に記載します。
